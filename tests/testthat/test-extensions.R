...
test_that("check docx extension", {
  ...
  file_abs_path <- try( docx_device(file = "my_plot.docxa", code = plot(rnorm(10))), silent = TRUE )
...
test_that("check pptx extension", {
  ...
  file_abs_path <- try( pptx_device(file = "my_plot.pptxa", code = plot(rnorm(10))), silent = TRUE )
  expect_is(object = file_abs_path, class = "try-error")
})
