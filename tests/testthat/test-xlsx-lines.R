...
test_that("segments don't have fill", {
  ...
  doc <- read_xml(file)
  fill_node <- xml_find_first(
    doc,
    ".//xdr:sp/xdr:spPr/a:solidFill",
    ns = xml_ns(doc)
  )
...
test_that("polygons without border", {
  ...
  doc <- read_xml(file)
  fill_color <- xml_find_first(
    doc,
    ".//xdr:sp/xdr:spPr/a:solidFill/a:srgbClr",
    ns = xml_ns(doc)
  )
...
dash_array <- function(...) {
  ...
  dash <- xml_find_first(
    doc,
    ".//xdr:sp/xdr:spPr/a:ln/a:prstDash",
    ns = xml_ns(doc)
  )
  dash
}
custom_dash_array <- function(...) {
  ...
  dash <- xml_find_all(
    doc,
    ".//xdr:sp/xdr:spPr/a:ln/a:custDash/a:ds",
    ns = xml_ns(doc)
  )
...
test_that("lty are ok", {
  ...
  expect_equal(
    custom_dash_array(lty = 4),
    c("100000", "300000", "400000", "300000")
  )
  expect_equal(xml_attr(dash_array(lty = 5), "val"), "lgDash")
  expect_equal(
    custom_dash_array(lty = 6),
    c("200000", "200000", "600000", "200000")
  )
  expect_equal(custom_dash_array(lty = "1F"), c("100000", "1500000"))
  expect_equal(
    custom_dash_array(lty = "1234"),
...
test_that("lty scales with lwd", {
  ...
  expect_equal(
    custom_dash_array(lty = 4, lwd = 2),
    c("200000", "600000", "800000", "600000")
  )
})
...
test_that("line join shapes", {
  ...
  doc <- read_xml(file)
  join_shape <- xml_find_first(
    doc,
    ".//xdr:sp/xdr:spPr/a:ln/a:round",
    ns = xml_ns(doc)
  )
