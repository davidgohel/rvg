...
- new method
  [`sheet_add_drawing.dml()`](https://davidgohel.github.io/rvg/reference/sheet_add_drawing.dml.md)
  to add vector graphics to an Excel sheet via
  [`officer::sheet_add_drawing()`](https://davidgohel.github.io/officer/reference/sheet_add_drawing.html).
  This provides a unified API for adding drawings to xlsx workbooks.

### Issues

- [`sheet_add_drawing.dml()`](https://davidgohel.github.io/rvg/reference/sheet_add_drawing.dml.md)
  now appends its anchor to the sheet’s existing drawing part (via
  [`officer::xlsx_drawing`](https://davidgohel.github.io/officer/reference/xlsx_drawing.html))
  instead of creating a second drawing part and a second `<drawing>`
  reference on the worksheet. Mixing `dml` with other
...
- Wrapped the two `Rf_error()` calls in `pptx_new_page()` and
  ...
  ([\#66](https://github.com/davidgohel/rvg/issues/66)). These are
  device callbacks where `Rf_error`’s longjmp is the correct signalling
  mechanism.
