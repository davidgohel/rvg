...
document, check for dependencies `d3.js` in the document. This is the svg device 
used by package [ggiraph](https://davidgohel.github.io/ggiraph).
* DrawingML for Microsoft PowerPoint with `dml_pptx` and for Microsoft Excel 
...
These raw XML outputs cannot be used *as is*. Functions `ph_with_vg()` and 
`ph_with_vg_at()` can be used with package `officer` to add vector graphics 
