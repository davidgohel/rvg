# Package index

## PowerPoint content

- [`dml()`](https://davidgohel.github.io/rvg/reference/dml.md) : Wrap
  plot instructions for DrawingML plotting in Powerpoint
- [`ph_with(`*`<dml>`*`)`](https://davidgohel.github.io/rvg/reference/ph_with.dml.md)
  : add a plot output as vector graphics into a PowerPoint object
- [`dml_pptx()`](https://davidgohel.github.io/rvg/reference/dml_pptx.md)
  : DrawingML graphic device for Microsoft PowerPoint

## Excel content

- [`sheet_add_drawing(`*`<dml>`*`)`](https://davidgohel.github.io/rvg/reference/sheet_add_drawing.dml.md)
  : Add a dml drawing to an Excel sheet
- [`dml_xlsx()`](https://davidgohel.github.io/rvg/reference/dml_xlsx.md)
  : DrawingML graphic device for Microsoft Excel
- [`xl_add_vg()`](https://davidgohel.github.io/rvg/reference/xl_add_vg.md)
  : add a plot output as vector graphics into an Excel object
