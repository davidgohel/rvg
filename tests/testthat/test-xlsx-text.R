...
test_that("text can be found", {
  ...
  doc <- read_xml(file)
  text_node <- xml_find_first(
    doc,
    ".//xdr:sp/xdr:txBody/a:p/a:r/a:t",
    ns = xml_ns(doc)
  )
...
test_that("special characters are escaped", {
  ...
  ns <- xml_ns(x)
  expect_equal(
    xml_text(xml_find_first(x, ".//xdr:sp/xdr:txBody/a:p/a:r/a:t", ns = ns)),
    "<&>"
  )
})
...
test_that("text color is written in fill attr", {
  ...
  expect_equal(
    xml_attr(
      xml_find_first(
        x,
        ".//xdr:sp/xdr:txBody/a:p/a:r/a:rPr/a:solidFill/a:srgbClr",
        ns = ns
      ),
      "val"
    ),
...
test_that("font sets weight/style", {
  ...
  x <- read_xml(file)
  ns <- xml_ns(x)
  rPr_latin <- xml_find_all(
    x,
    ".//xdr:sp/xdr:txBody/a:p/a:r/a:rPr/a:latin",
    ns = ns
  )
  rPr_cs <- xml_find_all(x, ".//xdr:sp/xdr:txBody/a:p/a:r/a:rPr/a:cs", ns = ns)
  expect_equal(
    xml_attr(rPr_latin, "typeface"),
    c("Times New Roman", "Arial", "Courier New")
  )
  expect_equal(
    xml_attr(rPr_cs, "typeface"),
    c("Times New Roman", "Arial", "Courier New")
  )
})
